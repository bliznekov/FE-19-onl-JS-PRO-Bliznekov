...
const fetchAllPostsWorker = function* () {
    ...
    try {
        const response: FetchAllPostsType = yield axios.get(
            `https://studapi.teachmeskills.by/blog/posts?limit=${1000}`
        );
...
const fetchMyPostsWorker = function* () {
    ...
    try {
        const response: FetchMyPostsType = yield call(
            sagaApi.get,
            `/blog/posts/my_posts`
        );
...
const postsSaga = function* () {
    yield all([spawn(fetchAllPostsWatcher), spawn(fetchMyPostsWatcher)]);
};
